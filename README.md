...
This repository contains the `Jupyter Notebooks` allowing to process numerical outputs of an `OpenModelica` model ([Fritzson et al., 2020](https://doi.org/10.4173/mic.2020.4.1)), developed with a TEA (Techno-Economic Assessment) approach ([Beerlage et al., 2024](https://doi.org/10.3384/ecp20780 )), which code is also available in Github under the name [CO2InnO-H2-CHP-Demonstrator](https://github.com/IKKUengine/CO2InnO-H2-CHP-Demonstrator). It is used for the study of decentralized energy systems (DES) based on hydrogen use (H₂) and using H₂ in an internal combustion engine for combined heat & power (ICE-CHP). 

The processing is required order to extract relevant data for a LCA (Life-Cycle Assessment) study of the system modeled from a TEA approach and allow to interactively generate relevant visualizations. The results dataset of the LCA and the dedicated processing notebooks are hosted in the [LCA-ScenarioResults-from-CO2InnO-H2-ICE-CHP-Demonstrator](https://github.com/Paul-Robineau/LCA-ScenarioResults-from-CO2InnO-H2-ICE-CHP-Demonstrator) companion repository.
